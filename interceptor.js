(function () {
    ...
    var path = require('path')
        , fs = require('fs');
    const { pathToFileURL } = require('url'); // Thêm h? tr? import ES6
    ...
    // run the original application entry point with ES6 module support

    if (appFile.endsWith('.mjs')) {
        import(pathToFileURL(appFile).href).catch(err => {
            console.error("Error loading ES6 module:", err);
        });
    } else {
        require(appFile);
    }
